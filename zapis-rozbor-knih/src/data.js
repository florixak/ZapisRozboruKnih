...
export const ikt_questions = [
  ...
  },
  {
    ...
    answer: [
      ...
      "- je to programovací jazyk od Microsoftu pro práci s datovými strukturami v Power BI, SQL Server Analysis Services a Power Pivot v Excelu",
      ...
      " ",
      "&lMnoho funkcí:",
      "- SUM, AVERAGE, MIN/MAX, ABS, PI, ROUND, CALCULATE, DISTINCT, COUNT...",
      " ",
      "&lSyntax:",
      "- název_funkce = výraz",
      " ",
      "&lPříklad:",
      "- Total Sales = SUM('Sales'[SalesAmount]) - sečte všechny prodeje",
      "- Total Sales = CALCULATE(SUM('Sales'[SalesAmount]), 'Sales'[Year] = 2021) - sečte prodeje za rok 2021",
      "- Column = CALCULATE(COUNT(List1[quantity]), FILTER(List1, List1[category_name] = 'Table'[category_name])) - spočítá počet záznamů v tabulce List1, které mají stejnou kategorii jako záznam v tabulce Table",
    ],
  },
  {
    id: 6,
  ...
  },
  {
    ...
    answer: [
      "- Proces, který se zaměřuje na strukturování a organizaci dat, tak aby bylo možné je efektivně spravovat, analyzovat a interpretovat",
      "- Zahrnuje tvorbu datových modelů, které popisují strukturu a vztahy mezi jednotlivými daty",
      " ",
      "&lTypy datových modelů:",
      "1. Konceptuální model",
      "- zaměřuje se na obecné struktury a vztahy mezi daty",
      "- nezabývá se tím, jak budou data uložena",
      " ",
      "2. Logický model",
      "- popisuje strukturu dat",
      "- určují se vlastnosti entit, atributy, vztahy mezi nimi",
      "- např.: Zákazník (entita) má atributy: jméno, příjmení, objednávky, a že každý zákazník může mít více objednávek",
      " ",
      "3. Fyzický model",
      "- popisuje, jak budou data uložena v databázi",
      "- definuje datové typy, indexy, primární a cizí klíče a další technické detaily",
      " ",
      "&lKlíčové koncepty:",
      "- Entity = objekt, o kterém chceme ukládat data, např. Zákazník, Produkt, má atributy, např. jméno, cena, datum",
      "- Atributy = vlastnoti entity, např. Student má atributy: ID, jméno, datum narození",
      "- Vztahy (relace) = spojení mezi entitami, např. Student může být přihlášený na kurz. Vztahy mohou být 1:1, 1:N, N:M",
      "- Primární klíč = unikátní identifikátor entity, např. ID studenta",
      "- Cizí klíč = klíč, který odkazuje na primární klíč jiné entity, slouží k propojení entit, např. Student má atribut ID kurzu jako cizí klíč",
      " ",
      "&lProces datového modelování:",
      "1. Shromáždění požadavků = zjistíme, jaké informace chceme ukládat",
      "2. Tvorba konceptuálního modelu = navrhneme obecnou strukturu, základní entity a vztahy",
      "3. Tvorba logického modelu = převedeme konceptuální model na strukturu dat, definujeme entity, atributy, vztahy",
      "4. Převod na fyzický model = převedeme logický model na strukturu databáze",
      "5. Optimalizace = zlepšíme výkon a efektivitu",
      " ",
      "&lNástroje pro datové modelování:",
      "- ER diagramy (Entity-Relationship) = diagramy, který zobrazují entity, atributy a vztahy mezi nimi",
      " ",
      "&lVýznam:",
      "- efektivní ukádání a správa dat",
      "- rychlý přístup k potřebným informacím",
      "- zajištění integrity a konzistence dat",
      "- zjednodušení analýzy a interpretace dat",
    ],
  },
  {
    id: 10,
  ...
  },
  {
    ...
    answer: [
      ...
      "- Unity, Unreal Engine, Godot = software pro tvorbu her, umí pracovat s 3D",
      " ",
      "&l3D formáty:",
      "- *.obj = jeden z nejstarších formátů, podporuje textury a materiály",
      "- *.fbx = podporuje animace, textury, materiály, pořád se vyvíjí",
      "- *.3ds = starší formát, podporuje textury a materiály",
      "- *.blend = formát pro Blender, podporuje všechny vlastnosti Blenderu",
      "- *.max = formát pro 3DS Max, podporuje všechny vlastnosti 3DS Max",
      "- *.stl = formát pro 3D tisk",
      " ",
      "&lZápisky:",
      "- ortogonála = kolmice",
      "- vzdálenost dvou bodů = výpočet pomocí Pythagorovy věty",
      "- RENDERING = proces, který ze zadaných paramtetrů vypočítává výsledný obraz",
      "- TRANSFORMACE = posun, rotace, změna velikosti (scale)",
      "- pravoúhlost = nejdůležitější věc pro kartézský systém souřadnic",
    ],
  },
  {
    id: 18,
    ...
    answer: [
      "&lSpline:",
      "- křivka, která je definována body",
      "- Obecná = nejde modifikovat",
      "- Bézierova křivka = možnost modifikace, body na křivce",
      "- NURBS křivka = možnost modifikace, body jsou mimo křivku",
      " ",
      "&lVertex (bod)",
      "- bod v prostoru = ×",
      "- základní prvek ve 3D modelování",
      " ",
      "&lEdge (hrana)",
      "- spojuje dva vrcholy a vytváří mezi nimi čáru = hrana",
      "- tvoří hrany polygonů",
      " ",
      "&lPolygon (ploška)",
      "- může mít n-hran",
      "- tvoří plochy objektů, určují tvar povrchu 3D objektu",
      "- nejčastěji bývá trojúhelníkový nebo čtyřúhelníkový",
      " ",
      "&lMesh (síť)",
      "- síť, struktura, která je složená z velkého množství vertexů, edge a polygonů",
      "- náročnější ná výkon počítače, složitější modely mohou mít vyšší počet polygonů",
      " ",
      "&lPatch (modifikovatelná síť)",
      "- síť, která je modifikovatelná, má body",
      "- pro tvorbu složitějších tvarů",
      "- je plynulejší, nemá ostré hrany",
      " ",
      "&lNURBS (Non-Uniform Rational B-Spline)",
      "- modifikátory jsou mimo patch",
      "- je početně náročnější, ale umožňuje lepší tvarování",
      " ",
      "&lZápisky:",
      "- INTERPOLACE = dopočet chybějících hodnot mezi dvěma známými body",
      "- bod nespojivosti = bod, kde se křivka nespojuje, hrozí nedogenerování 3D objektu",
    ],
  },
  {
    id: 19,
    ...
    answer: [
      "- Virtuální realita = odříznutí od reality",
      "- Rozšířená realita = jsem v reálném světě, ale s virtuálními prvky (Pokemon GO)",
      "- Hyper realita = spojení reálného a virtuálního světa, nerozlišitelné, nejde rozpoznat co je reálné a co ne",
      " ",
      "&lSmysly, které šálíme (dokážeme je nahradit):",
      "- zrach, sluch, hmat - vibrace, haptické rukavice",
      " ",
      "&lStereoskopie",
      "- zrakový vjem vyvolaný dvourozměrnou predlohou, do každého oka promítneme obrázek zvlášť, následě se obrazy spojí v mozku",
      " ",
      "&lVznik VR",
      "- v 19. století, pomocí stereoskopie, když byl vynalezen stereoskop, který umožňoval vidět 3D obraz",
      " ",
      "&lVývojové prostředí:",
      "- Unity = nejpopulárnější, jednoduché, podporuje všechny platformy",
      "- Unreal Engine = pokročilé, složitější, ale výkonnější",
      "- Godot = open-source, zdarma, jednoduché, ale spíše pro 2D",
      " ",
      "&lZískávání 3D objektů:",
      "- vlastní tvorba v 3D modelovacím softwaru",
      "- stažení z internetu, např. z Unity Asset Store, TurboSquid, ArtStation...",
      "- scanování reálných objektů pomocí 3D skeneru",
      "- fotogrammetrie = tvorba 3D modelů z fotografií",
      " ",
      "&lSkriptování:",
      "- C# = jazyk pro Unity, OOP jazyk",
      "- Datové typy v C# = int, float, String, bool, char, double, byte, short, long",
      "- Unity API = Application, Camera, GameObject, Transform, Rigidbody, Collider",
      "- Importování knihoven = using UnityEngine;",
      "- Základní metody = void Start(){} (volá se při spuštění), void Update(){} (volá se každý frame)",
      " ",
      "&lVyužití v praxi:",
      "- Zábava = hry, simulátory, filmy",
      "- Vzdělávání = výuka, trénink",
      "- Medicína = chirurgie, terapie, zbavení fobií",
      "- Architektura = návrhy, prohlídky",
      "- Konference = virtuální setkání, prezentace",
      "...",
    ],
  },
  {
    ...
    answer: [
      "&lIoT (Internet Of Things)",
      "- Internet věcí, jsou to všechna zařízení, která umí komunikovat mezi sebou, jsou připojená k internetu a měřit data pomocí senzorů",
      " ",
      "- Senzor = zařízení, které měří fyzikální veličiny a převádí je na elektrický signál",
      " ",
      "&lArchitektura IoT:",
      "- Senzory = sbírají data z okolí",
      "- Síť = přenáší data mezi zařízeními",
      "- Cloud = ukládá a zpracovává data",
      "- Uživatelské prostředí + bezpečnost = zobrazuje data a zajišťuje bezpečnost",
      " ",
      "&lTechnologie IoT:",
      "- Arduino = open-source platforma pro elektroniku a programování",
      "- Raspberry Pi = mini počítač, který lze použít jako server",
      "- ESP8266 = modul pro Wi-Fi komunikaci",
      "- Zigbee = bezdrátová technologie",
      "- LoRa = technologie pro komunikaci na velké vzdálenosti",
      "- Sigfox = technologie pro komunikaci na velké vzdálenosti",
      " ",
      "&lPříklady IoT zařízení:",
      "- Smart Home = chytré domácnosti, př.: chytré žárovky, termostaty, zámky, Google Home, Amazon Alexa",
      "- Chytrá města = města, která využívají IoT technologie, př.: osvětlení, hustota dopravy, parkování",
      "- Zdravotnictví = monitorování pacientů, sledování zdravotního stavu (např. chytré hodinky)",
      "- Manufakturenství = monitorování výroby, sledování stavu strojů",
      "- Zemědělství = monitorování půdy, zavlažování, sledování plodin",
      " ",
      "&lVýhody IoT:",
      "- automatizace věcí, které bychom museli dělat ručně",
      "- efektivita",
      "- snížení nákladů",
      ...
      "&lArduino UNO:",
      "    - napájení 5V",
      "    - 14 digitálních pinů, 6 analogových pinů, 6 PWM pinů (~)",
      "    - 9600 baudů = rychlost komunikace",
      "    - ATmega328P = mikrokontrolér",
      "    - USB-B",
      "&lESP8266:",
      "    - napájení 3,3V",
      "    - 11 digitálních pinů, 1 analogový pin, všechny piny jsou PWM (~)",
      "    - 115200 baudů = rychlost komunikace",
      "    - ESP8266 = mikrokontrolér",
      "    - micro USB, Wi-Fi",
    ],
  },
  {
...
export const vap_questions = [
  ...
  {
    id: 14,
    question: "HTML (HyperText Markup Language)",
    answer: [],
  },
  {
    id: 15,
